...
| 额度估算 | 估算单模型完整 7 天窗口容量时 | 按账号拟合模型消耗，展示容量区间、预测误差和 3 天对照 | 只读小时快照与使用日志 |
...
额度估算位于第三个 Tab，默认取最近 7 天，可切换 3 天或 24 小时。每个账号独立计算，丢弃重置所在小时及无效观测；所有模型参与稳健非负回归，默认展示有效总 Token 占比超过 10% 的模型。容量适用于样本期内相近的输入、输出和缓存比例，样本不足或模型用量无法区分时不提供容量。页面中的估算区间和预测误差用于判断稳定性，不能视为官方固定额度。
