...
Sub2API Lab (Sub2API拼车助手) 是基于 [Sub2API](https://github.com/Wei-Shaw/sub2api) 的独立月度对账工具，适合 Codex 多人拼车场景。`20x` 等高额度方案的单位成本通常低于 `1x` 或 `5x`，但单人往往用不完；多人拼车可以更充分地利用额度，但每个人的实际用量不同，平均分摊并不公平。本工具直接使用 Sub2API 已计算的用户系统消耗，按比例分摊当月的实际采购总成本。
