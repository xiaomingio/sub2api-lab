...
export function DataTable<T extends Record<string, unknown>>({ rows, columns, labels = {}, emptyText, defaultSort, renderCell }: DataTableProps<T>) {
  ...
  useEffect(() => { if (!openFilter) return; const close = () => setOpenFilter(null); document.addEventListener("click", close); return () => document.removeEventListener("click", close); }, [openFilter]);
  ...
  return <section className="data-table-shell" ref={scrollRef} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} aria-label="数据表格"><table className="data-table"><thead><tr>{keys.map((key) => { const info = columnInfo.find((item) => item.key === key) || { key, type: "text" as SortType }; const activeSort = sort?.key === key ? sort.direction : null; const currentFilter = columnFilters[key]; const allValues = new Set(rows.map((row) => displayValue(row[key]))); const filterActive = Boolean(currentFilter?.text?.trim() || currentFilter?.min?.trim() || currentFilter?.max?.trim() || currentFilter?.values && currentFilter.values.size !== allValues.size); return <th key={key}><div className="data-table-heading"><button className="data-table-label" type="button" title={key} onClick={() => toggleSort(key)}>{labelFor(key, labels)}</button><button className={`data-table-sort${activeSort ? " is-active" : ""}`} type="button" onClick={() => toggleSort(key)} aria-label={`按${labelFor(key, labels)}排序`}>{activeSort === "asc" ? "↑" : activeSort === "desc" ? "↓" : "↕"}</button><button className={`data-table-filter-button${filterActive ? " is-active" : ""}`} type="button" onClick={(event) => { event.stopPropagation(); const rect = event.currentTarget.getBoundingClientRect(); setFilterPosition({ top: rect.bottom + 6, left: Math.max(8, Math.min(rect.left, window.innerWidth - 320)) }); setOpenFilter((current) => current === key ? null : key); }} aria-label={`过滤${labelFor(key, labels)}`}>☰</button></div>{openFilter === key ? createPortal(<div className="data-filter-popover" style={filterPosition} onClick={(event) => event.stopPropagation()}><strong>{labelFor(key, labels)}</strong><FilterPanel keyName={key} type={info.type} filter={currentFilter} rows={rows} onChange={(value) => setColumnFilters((current) => ({ ...current, [key]: value }))} /></div>, document.body) : null}</th>; })}</tr></thead><tbody>{sortedRows.length === 0 ? <tr><td className="empty-cell" colSpan={Math.max(keys.length, 1)}>{emptyText}</td></tr> : <><tr aria-hidden="true"><td colSpan={keys.length} style={{ height: virtualized ? startIndex * rowHeight : 0, padding: 0, border: 0 }} /></tr>{visibleRows.map((row, index) => <tr key={`${startIndex + index}-${displayValue(row.id)}`}>{keys.map((key) => <td key={key}>{renderCell ? renderCell(row[key], key, row) : displayValue(row[key])}</td>)}</tr>)}<tr aria-hidden="true"><td colSpan={keys.length} style={{ height: virtualized ? Math.max(0, (sortedRows.length - startIndex - visibleRows.length) * rowHeight) : 0, padding: 0, border: 0 }} /></tr></>}</tbody></table></section>;
}
