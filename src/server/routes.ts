...
export function registerRoutes(app: FastifyInstance, options: AppOptions, handlers: RouteHandlers): void {
  ...
  for (const scope of scopes) {
    ...
    app.get(api("/api/usage-analysis"), { preHandler: requireAuth }, handlers.usageAnalysisApi);
    app.get(api("/api/quota-estimation"), { preHandler: requireAuth }, handlers.quotaEstimationApi);
    app.get(api("/api/quota-snapshots"), { preHandler: requireAuth }, handlers.quotaSnapshotsApi);
