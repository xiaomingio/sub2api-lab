...
export async function getUsageAnalysis(params: { db: Db; range: DateRange; filters?: AnalysisFilters; timezone: string; granularity: "hour" | "day" }): Promise<UsageAnalysis> {
  ...
  const accountTokenWindowCondition = `ul.created_at >= CASE WHEN NULLIF(quota_account.extra->>'codex_7d_reset_at', '') IS NOT NULL AND (quota_account.extra->>'codex_7d_reset_at')::timestamptz > CURRENT_TIMESTAMP THEN (quota_account.extra->>'codex_7d_reset_at')::timestamptz - make_interval(mins => COALESCE(NULLIF(quota_account.extra->>'codex_7d_window_minutes', '')::int, 10080)) ELSE CURRENT_TIMESTAMP - make_interval(mins => COALESCE(NULLIF(quota_account.extra->>'codex_7d_window_minutes', '')::int, 10080)) END`;
