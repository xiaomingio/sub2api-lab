 ...
 * 额度窗口由 Sub2API 写入 accounts.extra；账号卡片的本地 Token 复用 Sub2API 当前 7 天窗口口径统计 usage_logs。
 */
...
export async function getUsageAnalysis(params: { db: Db; range: DateRange; filters?: AnalysisFilters; timezone: string; now: Date; granularity: "hour" | "day" }): Promise<UsageAnalysis> {
  const values: unknown[] = [params.range.start, params.range.end];
  const accountWindowValues: unknown[] = [params.now];
  ...
  const accountTokenWindowCondition = `ul.created_at >= CASE WHEN NULLIF(quota_account.extra->>'codex_7d_reset_at', '') IS NOT NULL AND (quota_account.extra->>'codex_7d_reset_at')::timestamptz > $1::timestamptz THEN (quota_account.extra->>'codex_7d_reset_at')::timestamptz - make_interval(mins => COALESCE(NULLIF(quota_account.extra->>'codex_7d_window_minutes', '')::int, 10080)) ELSE $1::timestamptz - make_interval(mins => COALESCE(NULLIF(quota_account.extra->>'codex_7d_window_minutes', '')::int, 10080)) END`;
